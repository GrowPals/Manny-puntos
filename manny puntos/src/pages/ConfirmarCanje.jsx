...
const ConfirmarCanje = () => {
        ...
        const fetchProduct = async () => {
            try {
                ...
                if (user.puntos_actuales < prod.puntos_requeridos) {
                 ...
                 if (!prod.activo || (prod.tipo === 'producto' && prod.stock <= 0)) {
    ...
    }, [productoId, api, user, toast]);

    const handleConfirmarCanje = useCallback(async () => {
        if (!producto || error || !user) return;
        
        setIsSubmitting(true);
        try {
            const { nuevoSaldo, mensaje } = await api.registrarCanje({ cliente_telefono: user.telefono, producto_id: producto.id });
