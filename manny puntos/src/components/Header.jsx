    ...
    return (
        <RouterNavLink
            ...
            className={({ isActive }) => cn(
                "flex items-center justify-start text-base h-12 px-4 rounded-md transition-colors duration-200 w-full font-medium",
                isActive 
                    ? "btn-investment text-primary-foreground" 
...
const Header = () => {
    ...
    const AdminNavLinks = ({ onClick }) => (
        <>
            <NavLink to="/admin" onClick={onClick}><LayoutDashboard className="w-5 h-5 mr-3" /> Dashboard</NavLink>
            ...
            <NavLink to="/dashboard" onClick={onClick}><Shield className="w-5 h-5 mr-3" /> Vista Cliente</NavLink>
        </>
    );
    
    const ClientNavLinks = ({ onClick }) => (
        <>
            <NavLink to="/dashboard" onClick={onClick}><Gift className="w-5 h-5 mr-3" /> Canjear</NavLink>
            <NavLink to="/mis-canjes" onClick={onClick}><History className="w-5 h-5 mr-3" /> Mis Canjes</NavLink>
            <NavLink to="/sobre-manny" onClick={onClick}><Info className="w-5 h-5 mr-3" /> Sobre Manny</NavLink>
        </>
    );
                    ...
                    <>
                        ...
                        >
                            <div className="flex justify-between items-center mb-8">
                                <img src="https://i.ibb.co/LDLWZhkj/Recurso-1.png" alt="Manny Logo" className="h-10 w-auto" />
