...
const validateAndParseNumber = (value, defaultValue = 0) => {
    if (value === null || value === undefined || String(value).trim() === '') {
        return defaultValue;
    }
    const parsed = Number(value);
    return isNaN(parsed) || !isFinite(parsed) || parsed < 0 ? defaultValue : Math.floor(parsed);
};
...
const saveDb = (db) => {
  try {
    ...
    sanitizedDb.clientes = db.clientes.map(c => ({
      ...c,
      puntos_actuales: validateAndParseNumber(c.puntos_actuales),
      telefono: String(c.telefono).trim()
    }));
    sanitizedDb.lastUpdated = new Date().toISOString();
    sanitizedDb.version = DB_VERSION;
    localStorage.setItem(NOTION_DATA_KEY, JSON.stringify(sanitizedDb));
  } catch (error) {
...
export const exportMannyData = () => {
  try {
    ...
    throw new Error("La exportación de datos falló.");
  }
};

export const importMannyData = (file) => {
...
export const crearOActualizarCliente = async (clienteData) => {
  ...
  if (clienteData.id) { // Update
    ...
    if (index > -1) {
      db.clientes[index] = { ...db.clientes[index], ...clienteData, telefono: db.clientes[index].telefono, puntos_actuales: validateAndParseNumber(clienteData.puntos_actuales) };
    }
  } else { // Create
    ...
    const nuevoCliente = {
      ...
      puntos_actuales: validateAndParseNumber(clienteData.puntos_actuales),
...
export const asignarPuntosManualmente = async (clienteTelefono, puntos, concepto) => {
  ...
  const puntosAsignar = validateAndParseNumber(puntos, null);
  if (puntosAsignar === null) throw new Error("Cantidad de puntos inválida.");

  cliente.puntos_actuales = validateAndParseNumber(cliente.puntos_actuales) + puntosAsignar;
...
export const crearOActualizarProducto = async (productoData) => {
  ...
  const parsedData = {
    ...productoData,
    puntos_requeridos: validateAndParseNumber(productoData.puntos_requeridos),
    stock: productoData.tipo === 'producto' ? validateAndParseNumber(productoData.stock) : 999,
...
export const subirImagenProducto = async (archivo) => {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = (error) => reject(new Error('Error al leer el archivo de imagen: ' + error.message));
...
export const registrarCanje = async ({ cliente_telefono, producto_id }) => {
    const key = `${cliente_telefono}-${producto_id}`;
    if (pendingOperations.has(key)) {
        throw new Error('Operación en progreso, por favor espera.');
    }
    ...
    try {
        ...
        if (clienteIndex === -1) throw new Error('Cliente no válido.');
        if (productoIndex === -1) throw new Error('Producto no válido.');
        
        const cliente = db.clientes[clienteIndex];
        const producto = db.productos[productoIndex];

        if (validateAndParseNumber(cliente.puntos_actuales) < validateAndParseNumber(producto.puntos_requeridos)) {
            throw new Error('No tienes puntos suficientes.');
        }

        if (producto.tipo === 'producto' && validateAndParseNumber(producto.stock) <= 0) {
            throw new Error('Producto agotado durante la transacción.');
        }

        if (!producto.activo) throw new Error('Esta recompensa no está disponible actualmente.');

        cliente.puntos_actuales -= validateAndParseNumber(producto.puntos_requeridos);
        ...
        const nuevoCanje = {
            ...
            producto_id,
            tipo: producto.tipo,
            puntos_usados: validateAndParseNumber(producto.puntos_requeridos),
            fecha: new Date().toISOString(),
