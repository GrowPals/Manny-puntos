...
const ClientForm = ({ client, onFinished }) => {
    // If we're creating, we need an empty object with an explicit ID field for Supabase `upsert`
    const initialState = client ? { ...client } : { id: undefined, nombre: '', telefono: '', puntos_actuales: 0 };
        ...
        <DialogContent className="bg-card border-border text-foreground">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 py-4">
                <div className="space-y-2">
                    ...
                    <Input id="nombre" name="nombre" value={formData.nombre} onChange={handleChange} placeholder="Nombre completo con espacios" required className="h-12 text-base" />
                </div>
                <div className="space-y-2">
...
const AdminClientes = () => {
    const { getTodosLosClientes } = useSupabaseAPI();
